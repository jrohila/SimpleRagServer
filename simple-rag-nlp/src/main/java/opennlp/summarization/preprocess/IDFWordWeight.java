...
public class IDFWordWeight implements WordWeight {
  ...
  private void load(String fileName) throws IOException {
    InputStream in = IDFWordWeight.class.getResourceAsStream(fileName);
    if (in == null) {
      // Resource not found; default to empty IDF (all weights=1). Avoid NPE and proceed.
      System.err.println("[opennlp-summarizer] Warning: IDF resource '" + fileName + "' not found on classpath. " +
              "Using default weights (1.0) for all terms.");
      return;
    }
    try (InputStream autoClose = in;
         LineNumberReader lnr = new LineNumberReader(new InputStreamReader(autoClose))) {
