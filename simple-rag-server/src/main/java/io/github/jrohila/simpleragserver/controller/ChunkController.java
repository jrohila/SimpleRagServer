...
public class ChunkController {
    ...
    // Create
    @Post
    public HttpResponse<ChunkEntity> createChunk(@QueryValue(value = "collectionId", defaultValue = "") String collectionId, @Body ChunkEntity chunk) {
    ...
    // Read by id
    @Get("/{collectionId}/{id}")
    public HttpResponse<ChunkEntity> getChunk(@PathVariable String collectionId, @PathVariable String id) {
        ...
        return chunkOpt.map(HttpResponse::ok).orElseGet(HttpResponse::notFound);
    }

    // List (optionally filter by documentId)
    @Get
        public HttpResponse<List<ChunkEntity>> listChunks(
            @QueryValue(value = "page", defaultValue = "0") int page,
            @QueryValue(value = "size", defaultValue = "20") int size,
            @Nullable @QueryValue(value = "collectionId") String collectionId,
            @Nullable @QueryValue(value = "documentId") String documentId
        ) {
    ...
    // Update
    @Put("/{collectionId}/{id}")
    public HttpResponse<ChunkEntity> updateChunk(@PathVariable String collectionId, @PathVariable String id, @Body ChunkEntity chunk) {
        ...
        return saved.map(HttpResponse::ok).orElseGet(HttpResponse::notFound);
    }

    // Delete by id
    @Delete("/{collectionId}/{id}")
    public HttpResponse<Void> deleteChunk(@PathVariable String collectionId, @PathVariable String id) {
    ...
    // Delete all
    @Delete("/{collectionId}")
    public HttpResponse<Void> deleteAllChunks(@PathVariable String collectionId) {
    ...
    @Delete("/by-document/{collectionId}/{documentId}")
    public HttpResponse<Void> deleteChunksByDocument(@PathVariable String collectionId, @PathVariable String documentId) {
